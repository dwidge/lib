...
export const replaceItemById = (list, item) =>
  list.map((o) => (o.id === item.id ? item : o));

export const dropItemById = (list, id) => list.filter((o) => o.id !== id);

export const dropIfIncluded = (as, bs, isEq) =>
  as.filter((a) => !bs.find((b) => isEq(a, b)));
...
export const range = (beg, end) => Array(end - beg).map((_, i) => i + beg);
